...
export const renderUserInfo = (user) => {
  ...
  // kuullanıcı isimlerini ekrana bas
  ele.user_name.innerText = user.name;
  ele.user_tag.innerText = '@' + user.profile;
};
// tweetin medya içeriğini alıp içeriğe göre htmle dönüştürecek
...
//1 data:twetler
//2 outlet: hangi elmentin içerisine göndericez
export const renderTimeline = (data, outlet, user) => {
  console.log(data.timeline[1]);
  // herbir tweet için outline alanına bir tweet divi bas
  outlet.innerHTML = data.timeline.map(
    (tweet) => ` 
    <div class="tweet">
    <img id="user-img" src="${tweet[user] ? tweet[user].avatar
        : '/images/user.png'}" />
    <div class="body">
      <div class="user">
      ${tweet[user]
...
//ekrana tweet detay sayfasınnı basar
export const renderDetail = (tweet, user) => {
  ...
  ele.main.innerHTML = `
    ...
    <form id="comment-form">
      <img src="${user.avatar}"/>
      <input placeholder="Yanıtı Gönder" type="text">
